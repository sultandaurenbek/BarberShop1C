...
Процедура ПланировщикПередСозданием(Элемент, Начало, Конец, ЗначенияИзмерений, Текст, СтандартнаяОбработка)
	...
	ЕстьЗапись =  ПроверитьДоступностьЗаписи(Салон,Начало,ЗначенияИзмерений["Сотрудник"]);
...
Функция ПроверитьДоступностьЗаписи (Салон,ДатаНачало,Сотрудник)
	...
	Запрос.УстановитьПараметр("ДатаОкончания", КонецДня(ДатаНачало));
